...
# -----------------------------
# Load Keras models WITHOUT TensorFlow
# -----------------------------
@st.cache_resource
def load_keras_model(path):
    try:
        if os.path.exists(path):
            return keras.saving.load_model(path)  # <-- TF-free loading
...
def load_pickle(path):
    try:
        ...
        st.warning(f"Failed to load pickle {path}: {e}")
    return None

# Load models
mri_model = load_keras_model(MRI_MODEL)
drawing_model = load_keras_model(DRAWING_MODEL)
    ...
    if file is None:
        ...
        if mod == "MRI Image":
            ...
            if mri_model:
                ...
                p = float(out.squeeze()) if out.ndim == 1 else float(out[0][-1])
            else:
        ...
        # ---------------- Drawing ----------------
            ...
            try:
                ...
                st.error("Invalid drawing image.")
                st.stop()
